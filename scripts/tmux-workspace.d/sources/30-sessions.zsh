list-sessions() {
  tmux list-sessions -F $'#S\t#{session_windows} windows\t#{session_attached} attached' 2>/dev/null || true
}
...
pick-session() {
  ...
  require-fzf
  cmd="tmux list-sessions -F '#S	#{session_windows} windows	#{session_attached} attached'"
  ...
  selection="$(list-sessions | FZF_DEFAULT_OPTS="$FZF_OPTS" \
    fzf --ansi --no-hscroll --height="$FZF_HEIGHT" --layout=reverse --border \
      ...
      --bind "ctrl-d:execute-silent(tmux kill-session -t {1})+reload($cmd)+clear-query" \
